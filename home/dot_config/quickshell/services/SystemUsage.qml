...
Singleton {
    ...
    Process {
        ...
        running: true
        command: ["nu", "-c", "ls /sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/temp*_input | get name | each { |file| (open $file | into int) / 1000 } | str join ' '"]
        stdout: SplitParser {
            onRead: data => {
                const temps = data.trim().split(" ");
                const sum = temps.reduce((acc, d) => acc + parseFloat(d), 0);
                root.cpuTemp = sum / temps.length;
            }
        }
    }
...
Process {
    ...
    stdout: SplitParser {
        onRead: data => {
            root.gpuPerc = parseInt(data.trim(), 10) / 100;
        }
    }
}
...
Process {
    ...
    stdout: SplitParser {
        onRead: data => {
            root.gpuTemp = parseFloat(data.trim());
        }
    }
}
}
